...
CASES = [
    ("fat", dict(n_samples=2048, inner=(64, 64), sample_chunk=64, batch=32, block=16, inflight=16)),
    (
        "grib",
        dict(n_samples=2048, inner=(128, 128), sample_chunk=1, batch=32, block=256, inflight=32),
    ),
]
...
def main() -> None:
    ...
    print(
        f"{'case':6s} {'engine':12s} {'samples':>8s} {'sec':>7s} "
    ...
    for regime, c in CASES:
        ...
        for res in (
            ...
            run_insitu(
                ...
                geom,
                batch_size=c["batch"],
                block_chunks=c["block"],
                max_inflight=c["inflight"],
            ),
        ):
            res.regime = regime
            log(res)
            print(
                f"{regime:6s} {res.label:12s} {res.n_samples:8d} {res.seconds:7.3f} "
