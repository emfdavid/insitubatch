...
def peak_rss_mb() -> float:
    ...
    single-process suite it sticks at the largest config seen so far (a large-budget
    heap-cache run inflates every later row). Prefer the per-row ``rss_anon_mb`` below.
    """
...
def rss_breakdown_mb() -> tuple[float, float]:
    ...
    **RssFile** (file-backed mmap — e.g. the pool's mmap'd ``.npy`` slots, which are
    reclaimable page cache, not heap). ru_maxrss conflates the two; this is sampled
    per row so an mmap-backed cache shows a bounded heap even when total RSS looks large.
